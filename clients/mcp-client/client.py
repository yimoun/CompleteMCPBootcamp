...
class MCPClient:
    ...
    async def process_query(self, query: str) -> str:
        ...
        response = self.genai_client.models.generate_content(
            model='gemini-flash-latest',  # Specifies which Gemini model to use (using flash-latest for free tier compatibility)
                ...
                for part in candidate.content.parts:
                    if isinstance(part, types.Part):  # Check if part is a valid Gemini response unit
                        if part.function_call:  # If Gemini suggests a function call, process it
                            ...
                            response = self.genai_client.models.generate_content(
                                model='gemini-flash-latest',  # Use the same model (using flash-latest for free tier compatibility)
                                contents=[
